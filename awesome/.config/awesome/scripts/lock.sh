...
then
    ...
    R=$(file $HOME/dotfiles/awesome/.config/awesome/screen-lock.png | grep -o '[0-9]* x [0-9]*')
    RX=$(echo $R | cut -d' ' -f 1)
    RY=$(echo $R | cut -d' ' -f 3)
    ...
    do
        ...
        convert /tmp/screen.png $HOME/.config/awesome/screen-lock.png -geometry +$PX+$PY -composite -matte  /tmp/screen.png
        echo "done"
    done
fi
